...
def _get_proximity_sorted(origin_id: Any, candidates: List[Any], type_object: str, app_set: Any, user_set: Any, infrastructure: Any) -> List[Any]:
...
def resolve_targets(first_event: Dict[str, Any], sub_action: Dict[str, Any], app_set: Any, user_set: Any, infrastructure: Any, sim_set: Any) -> List[Any]:
    ...
    if mode == 'group':
        ...
        if strategy == 'list':
            ...
            return random.sample(all_candidates, min(num_elements, len(all_candidates)))
            ...
            if needed > 0 and candidates:
                res.extend(random.sample(candidates, min(needed, len(candidates))))
            return res
            ...
            if original_id in all_candidates:
                ...
                original_id = random.choice(all_candidates)
                res.append(original_id)
                
            sorted_candidates = _get_proximity_sorted(original_id, all_candidates, type_object, app_set, user_set, infrastructure)
            needed = num_elements - len(res)
            if needed > 0:
            ...
            origin = random.choice(all_candidates)
            res = [origin]
            sorted_candidates = _get_proximity_sorted(origin, all_candidates, type_object, app_set, user_set, infrastructure)
            needed = num_elements - len(res)
            if needed > 0:
                res.extend(sorted_candidates[:needed])
            return res
